...
class CoinFragment :
    ...
    override fun getPresenterFactory(saveInstanceState: Bundle?) = CoinPresenter(
        requireArguments().getInt(Extra.TYPE, 1),
        saveInstanceState
    )
    ...
    override fun displayRegister(register: Register?) {
        mBalance?.text =
            requireActivity().getString(R.string.rub, String.format("%.2f", register?.coinBalance))
    }
    ...
    class CreateOrDeleteOperationDialog : DialogFragment() {
        ...
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            ...
            val mTittle = view.findViewById<TextInputEditText>(R.id.edit_tittle)
            ...
            mTittle.setText(obj.tittle)
            mComment.setText(obj.comment)
            mCoin.setText(fixNumeric(String.format("%.2f", obj.coin)))
            return MaterialAlertDialogBuilder(requireActivity())
                ...
                .setPositiveButton(R.string.button_ok) { _: DialogInterface?, _: Int ->
                    try {
                        obj.tittle = mTittle.text.toString()
