...
class FinanceWalletsAdapter(private var data: List<FinanceWallet>) :
    RecyclerView.Adapter<FinanceWalletsAdapter.Holder>() {
    private var clickListener: ClickListener? = null
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Holder {
        return Holder(
            LayoutInflater.from(parent.context)
                .inflate(R.layout.item_finance_wallet, parent, false)
        )
    }
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        ...
        holder.coins.text = holder.itemView.context.getString(
            R.string.coin_param,
            String.format(Utils.appLocale, "%.2f", item.coins)
        )
    ...
    fun setData(data: List<FinanceWallet>) {
        this.data = data
        notifyDataSetChanged()
    }

    interface ClickListener {
        fun onClick(position: Int, value: FinanceWallet)
        fun onLongClick(position: Int, value: FinanceWallet): Boolean
        fun onDelete(position: Int, value: FinanceWallet)
        fun onEdit(position: Int, title: String?)
    }

    class Holder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val animatorHeight: ValueAnimator = ValueAnimator.ofInt(Utils.dp(100f), Utils.dp(240f))
        val animatorAlpha: ValueAnimator = ValueAnimator.ofFloat(0f, 1f)

        val animatorHeightReversed: ValueAnimator =
            ValueAnimator.ofInt(Utils.dp(240f), Utils.dp(100f))
