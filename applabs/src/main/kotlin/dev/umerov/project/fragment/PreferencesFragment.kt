...
import dev.umerov.project.picasso.PicassoInstance.Companion.clear_cache
...
class PreferencesFragment : AbsPreferencesFragment(), PreferencesAdapter.OnScreenChangeListener,
    ...
    private fun createRootScreen() = screen(requireActivity()) {
        ...
        subScreen("snake_settings") {
            titleRes = R.string.snake
            seekBar("snake_volume_value") {
                default = 10
                min = 0
                max = 10
                titleRes = R.string.volume_label
            }
            seekBar("snake_refresh_ms") {
                default = 200
                min = 50
                max = 300
                step = 50
                titleRes = R.string.snake_refresh_ms
            }
            switch("enable_accelerometer") {
                defaultValue = true
                titleRes = R.string.enable_accelerometer
            }
        }
    }
    ...
    companion object {
        ...
        fun cleanCache(context: Context, notify: Boolean) {
            Includes.stores.projectStore().drop().blockingSubscribe()
            try {
                clear_cache()
