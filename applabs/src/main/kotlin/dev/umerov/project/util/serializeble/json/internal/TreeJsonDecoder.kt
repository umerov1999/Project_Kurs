...
import dev.umerov.project.util.serializeble.json.booleanOrNull
import dev.umerov.project.util.serializeble.json.contentOrNull
import dev.umerov.project.util.serializeble.json.double
import dev.umerov.project.util.serializeble.json.float
import dev.umerov.project.util.serializeble.json.int
import dev.umerov.project.util.serializeble.json.internal.lexer.StringJsonLexer
import dev.umerov.project.util.serializeble.json.internal.lexer.lenientHint
import dev.umerov.project.util.serializeble.json.long
...
private sealed class AbstractJsonTreeDecoder(
    ...
    inline fun <reified T : JsonElement> cast(value: JsonElement, descriptor: SerialDescriptor): T =
        cast(value, descriptor.serialName) { renderTagStack() }

    inline fun <reified T : JsonElement> cast(
        value: JsonElement,
        serialName: String,
        tag: String
    ): T = cast(value, serialName) { renderTagStack(tag) }
    ...
    private inline fun <T : Any> getPrimitiveValue(
        tag: String,
        primitiveName: String,
        convert: JsonPrimitive.() -> T?
    ): T {
    ...
    override fun decodeTaggedChar(tag: String): Char =
        getPrimitiveValue(tag, "char") { content.single() }
...
private open class JsonTreeDecoder(
    ...
    override fun beginStructure(descriptor: SerialDescriptor): CompositeDecoder {
        ...
        if (descriptor === polyDescriptor) {
            return JsonTreeDecoder(
                json,
                cast(currentObject(), polyDescriptor),
