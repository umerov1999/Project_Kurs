package dev.umerov.project.util.serializeble.msgpack
...
import dev.umerov.project.util.serializeble.msgpack.internal.BasicMsgPackDecoder
import dev.umerov.project.util.serializeble.msgpack.internal.BasicMsgPackEncoder
import dev.umerov.project.util.serializeble.msgpack.internal.InlineDecoderHelper
import dev.umerov.project.util.serializeble.msgpack.internal.InlineEncoderHelper
import dev.umerov.project.util.serializeble.msgpack.internal.MsgPackDecoder
import dev.umerov.project.util.serializeble.msgpack.internal.MsgPackEncoder
import dev.umerov.project.util.serializeble.msgpack.stream.toMsgPackArrayBuffer
import dev.umerov.project.util.serializeble.msgpack.stream.toMsgPackBufferedSource
...
open class MsgPack @JvmOverloads constructor(
    ...
    fun <T> decodeFromByteArrayEx(
        ...
        if (!ProjectNative.isNativeLoaded || bytes.size < 4 || bytes[0] != 0x02.toByte() || bytes[1] != 0x4C.toByte() || bytes[2] != 0x5A.toByte() || bytes[3] != 0x34.toByte()) {
    ...
    fun <T> encodeToByteArrayEx(
        ...
        val encoder = MsgPackEncoder(
            BasicMsgPackEncoder(
                ...
                serializersModule,
                inlineEncoders = inlineEncoders,
                compressed = ProjectNative.isNativeLoaded
            )
        )
        kotlin.runCatching {
            encoder.encodeSerializableValue(serializer, value)
